...
export const POST = withApiHandler({
  ...
  if (generateWithAI) {
    ...
    // Track token usage for AI generation
  ...
  // Audit log
  await logAction(user.id, 'plan.create', 'business_plans', {
    planId: plan.id,
    title,
  ...
  await trackEvent({
    ...
    source: 'api',
    status: 'info',
    message: `Business plan created: ${title}`,
    data: { planId: plan.id, generateWithAI: !!generateWithAI },
  }).catch(() => {})
